...
export class IGDB {
  ...
  async scrape(game: Game, system: System): Promise<Game> {
    ...
    const buildImageMediaReq = (id: string, mediaType: string): GameMedia | null => (id ? {
    ...
    const metadata = [
      { key: "developer", value: gameResponse.involved_companies?.find(company => company.developer)?.company?.name },
      { key: "publisher", value: gameResponse.involved_companies?.find(company => company.publisher)?.company?.name },
      { key: "description", value: gameResponse.summary },
      { key: "genre", vaolue: gameResponse.genres?.[0]?.name }
    ].filter(m => m.value).reduce((acc, { key, value }) => ({ ...acc, [key]: value }), {})
