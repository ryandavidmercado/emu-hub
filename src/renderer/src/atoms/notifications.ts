...
const add = atom(null, (_, set, notification: PartialBy<Notification, 'id'>) => {
...
const update = atom(null, (get, set, update: Partial<Notification> & { id: string }) => {
  ...
  const event = new CustomEvent('notification-display', {
    detail: {
      ...newNotif,
