...
export interface Options { text: string | ReactNode, defaultSelected?: number, options: Option[], canClose?: boolean }
...
export const useSelect = () => {
  ...
  return {
    getSelection: async ({ text, defaultSelected, options, canClose = true }: Options): Promise<string | null> => {
      setOptions(options);
...
const ConfirmationModal = ({ open, text, selected, setSelected, options, canClose, className }: SelectModalProps & { className?: string }) => {
  useOnInput((input) => {
    switch (input) {
      case Input.A:
