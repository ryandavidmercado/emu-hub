  ...
  const handleGamepads = () => {
    ...
    for (const gamepad of gamepads) {
      ...
      gamepad.axes.forEach((axis, i) => {
        if(i === 0 && axis < -.6) pressedDirections.add(Input.LEFT)
        if(i === 0 && axis > .6) pressedDirections.add(Input.RIGHT)
        if(i === 1 && axis < -.6) pressedDirections.add(Input.UP)
        if(i === 1 && axis > .6) pressedDirections.add(Input.DOWN)
      })
