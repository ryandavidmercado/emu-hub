...
export const useKeepVisible = (
  ...
  useEffect(() => {
    ...
    const parentBox =
      (overrideParentBox as DOMRect) ?? ref.current.parentElement.getBoundingClientRect()
...
const handleHorizontal = (
  ...
  if (elementBox.right > parentBox.right) {
    elementRef.current.parentElement.scrollLeft =
      elementBox.right - parentBox.right + padding + elementRef.current.parentElement.scrollLeft
  }
  if (elementBox.left < parentBox.left) {
    elementRef.current.parentElement.scrollLeft =
      elementBox.left - parentBox.left + elementRef.current.parentElement.scrollLeft - padding
  }
}
...
const handleVertical = (
  ...
  elementRef.current.parentElement.scrollTop =
    elementBox.top - parentBox.top + elementRef.current.parentElement.scrollTop - padding
